...
class WriteTemplate(object):
    """
        ...
        It defines:
            ...
            :methods:
                ...
                | get_project_status - Getter for project status
    ...
    def write(self, project_data, verbose=False):
        ...
        self.__project_status = all(
            [bool(project_data['templates']), bool(project_data['name'])]
        )
        if self.__project_status:
            for pro_item in project_data['templates'].keys():
                check_sources = any(
                    [
                        '.c' in project_data['templates'][pro_item][0],
                        '.h' in project_data['templates'][pro_item][0]
                    ]
                )
                if check_sources:
                    module = "{0}/{1}".format(
                        current_dir, project_data['templates'][pro_item][0]
                    )
                else:
                    module = "{0}/{1}".format(
                        build_dir, project_data['templates'][pro_item][0]
                    )
                verbose_message(
                    WriteTemplate.VERBOSE, verbose,
                    'Write project module file', module
                )
                project = {'PRO': "{0}".format(project_data['name'])}
                template = Template(project_data['templates'][pro_item][1])
                if template:
